...
class Chicken {
    ...
    getTransform(intCtx, matrix) {
        var transformedChicken = transformEntity(this.game, this, matrix);

        ////console.log(transformedChicken);

        if (transformedChicken.y <= CANVAS_HEIGHT / 2) {
            //intCtx.fillRect(transformedChicken.x - 2, transformedChicken.y - 2, 4, 4);
            //console.log(p);

            return new Chicken(this.game, PRE_SCALE * -(CANVAS_WIDTH / 2 - transformedChicken.x) / ((400 - transformedChicken.y) + 2) + CANVAS_WIDTH / 2, transformedChicken.y);
        } else return null;
    }
